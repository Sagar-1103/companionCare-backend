...
const storeReport = async(call,cb)=>{
    try {
        ...
        let genReport;
        if(role==="patient"){
            genReport = await Report.create({report:reportUrl,name,patientId:id});
        }
        if(role==="doctor"){
            genReport = await Report.create({report:reportUrl,name,doctorId:id});
        }
        if(!genReport){
            return cb({
...
export {storeReport,getReportForPatient,getReportForDoctor};